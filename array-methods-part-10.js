...
testCode(
  "91) Shirts in a laundry batch listed as “white”.",
...
testCode(
  "92) Combine all notes into one list : ",
...
testCode(
  "93) Check if any item is marked “fragile : ",
...
testCode(
  "94) Verify that all letters are lowercase : ",
...
const ingredients = (list1, list2, list3) => {
  return [list1, list2, list3].flatMap((x) => x).filter(
    (item, index, dishes) => {
...
testCode(
  "100) List of distinct songs hummed by children on a bus ride : ",
  distinctRhymes([
    "wheels on the bus",
    "wheels on the bus",
    "row row row",
